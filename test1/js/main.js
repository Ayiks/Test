...
function loadCSVFile (event) {
    ...
    if (csvfile) {
        var original_csv   = document.getElementById("original");
        var processed_csv = document.getElementById("processed");
        var only_zeros    = true;
        ...
        reader.onload  = function () {
            ...
            //dynamic two-dimensional array (matrix) and store all values
            ...
            for (var row=1; row<lines.length; row++) {
                ...
                for (var col=0; col<values.length; col++) {
                    ...
                    original_csv.value += values[col];
                    original_csv.value += col+1 < values.length ? delim : "";
                ...
                original_csv.value += row+1 < lines.length ? "\n" : "";
            ...
            while (bad_lines.length > 0 && !only_zeros) {
                ...
                while (matrix[bad_l].includes(0)) {
                    ...
                    if (good != 0) {
                        ...
                        console.log("No close neighbors! Searching...");
            ...
            for (var my=0; my<matrix.length; my++) {
                for (var mx=0; mx<matrix[my].length; mx++) {
                    processed_csv.value += matrix[my][mx];
                    processed_csv.value += mx+1 < matrix[my].length ? delim : "";
                }
                processed_csv.value += my+1 < matrix.length ? "\n" : "";
            }
        }
        ...
        reader.onloadend = function() {
            if (reader.error) {
                alert("File not loaded successfully. Please try again.");
                console.error("Unsuccessful load of " + csvfile.name + ": " + reader.error.message);
            }
            else {
                // Get the textarea's value and create the html
                var html = original_csv.value.replace(/\n/g,"<br>").replace(/0/g,"<b>0</b>");
                document.getElementById("text-div").innerHTML = html;

                // and similar for the processed textarea
                html = only_zeros ? "<br>&nbsp; ¯\\_(ツ)_/¯ <br><br>" : processed_csv.value.replace(/\n/g,"<br>");
